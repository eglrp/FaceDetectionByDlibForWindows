namespace DlibSharp
{
    ...
    public class DnnSemanticSegmentation : IDisposable
    {
        IntPtr detector = IntPtr.Zero;
        IntPtr dets = IntPtr.Zero;

        void ReleaseDetector() { if (detector != IntPtr.Zero) { NativeMethods.dlib_dnn_semantic_segmentation_delete(detector); detector = IntPtr.Zero; } }
        void ReleaseDets() { if (dets != IntPtr.Zero) { NativeMethods.vector_Rect_delete(dets); dets = IntPtr.Zero; } }

        public DnnSemanticSegmentation(string mmodHumanFaceDetectorDataFilePath)
        {
            if (IntPtr.Size == 4)
            {
            ...
            detector = NativeMethods.dlib_dnn_semantic_segmentation_construct(mmodHumanFaceDetectorDataFilePath);
        }
        ...
        public System.Drawing.Rectangle[] DetectFaces(MatrixRgbPixel inputImage)
        {
            ...
            try
            {
                ...
                NativeMethods.dlib_dnn_semantic_segmentation_operator(detector, inputImage.DlibMatrixRgbPixel, dets);
                unsafe
                {
        ...
        ~DnnSemanticSegmentation() { Dispose(false); }
        #endregion
    }
    ...
    internal static partial class NativeMethods
    {
        ...
        extern internal static IntPtr dlib_dnn_semantic_segmentation_construct(string mmodHumanFaceDetectorDataFilePath);

        [DllImport(DlibExternDllPath, CallingConvention = CallingConvention.Cdecl)]
        extern internal static void dlib_dnn_semantic_segmentation_delete(IntPtr obj);

        [DllImport(DlibExternDllPath, CallingConvention = CallingConvention.Cdecl)]
        extern internal static void dlib_dnn_semantic_segmentation_operator(IntPtr obj, IntPtr image, IntPtr dst);
    }
}
